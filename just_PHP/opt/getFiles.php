...
$date=isset($_POST['DD'])?validateFilename($_POST['DD']):"";
...
if($date!="" && $classid!="" && isset($classname[$classid-1]))
{
   ...
   $add="..\\".$student_works_dir."$class\\$date\\";

   //$add="..\\".$student_works_dir."$class\\$date\\*.c";
   //$list=glob($add);
   ...
   if ($handle = opendir($add))
   {
      while (false !== ($file = readdir($handle)))
      {
         ...
         if (is_file($add.$file)==TRUE && $f["extension"]==$type  )//  && ($admin==1 || strstr($file,$username) || strstr($file,"ALL_") || strstr($file,"_EDITABLE_") ))
         { 
            $list[]=$file;
         }
