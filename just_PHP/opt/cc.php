...
if($strCODE){
   $retArr=Array();
   $path="..\\".$student_works_dir.$strCN."\\".$datedir;
...
//   echo '{"'.$retArr['resrun'].'","'.convertMixedToUtf8($retArr['rescomp']).'"}';
//   echo '{"'.$retArr['resrun'].'","'.iconv("GBK","UTF-8//IGNORE",$retArr['rescomp']).'"}';

   //$resrun=iconv("gbk","utf-8//IGNORE",$retArr["resrun"]);		//如有非UTF-8的中文，json_encode会失败。
   //if($resrun) 
   //$retArr["resrun"]=convertMixedToUtf8($retArr["resrun"]);//?base64_encode($resrun):base64_encode($retArr["resrun"]);

   //$rescomp=iconv("gbk","utf-8//IGNORE",$retArr["rescomp"]);
   //if($rescomp) 
   //$retArr["rescomp"]=convertMixedToUtf8($retArr["rescomp"]);
//   echo $retArr["rescomp"]."|||||".$retArr["resrun"];
//var_dump($retArr);
